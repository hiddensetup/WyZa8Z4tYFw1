            ...
            if (busy) {
              conversations_area
                ...
                .html(
                  `<span data-agent="${
                    busy.id
                  }" style="box-shadow: 0px 1px 4px -1px rgba(0, 0, 0, 0.65);border-radius: .8REM;position: absolute;bottom: 70px;background: var(--blue-root-color);color: aliceblue;padding: 3px 10px;font-size: 0.8rem;"><b>${
                    busy.first_name
      ...
      SBF.ajax(requests, (response) => {
        ...
        if (i < user_ids_length - 1) {
          ...
          if (user_ids_sms.length) {
            ...
            if (settings[3]) SBF.ajax({ function: 'reports-update', name: settings[3], value: message + ' | ' + user_ids_length });
