...
function sb_get_users(
    ...
    // $users = sb_db_get(
    //     "SELECT u.*, ud.value AS phone FROM sb_users u " .
    //     "LEFT JOIN sb_users_data ud ON u.id = ud.user_id AND ud.slug = 'phone' " .
    //     $query .
    //     sb_routing_and_department_db("sb_conversations", true) .
    //        ($main_field_sorting
    //             ? " ORDER BY " .
    ...
    //             sb_db_escape($sorting[1])
    //             : "") .
    //             " LIMIT " . intval(sb_db_escape($pagination, true)) * 25 . ",25",
    //             false
    //         );
...
function sb_get_conversations_users($conversations)
{
    if (count($conversations) > 0) {
        ...
        $result = sb_db_get(
            ...
            WHERE sb_conversations.id IN " . $code,
            false
        );
        ...
        for ($i = 0; $i < count($conversations); $i++) {
            ...
            for ($j = 0; $j < count($result); $j++) {
                if ($conversation_id == $result[$j]["conversation_id"]) {
                    ...
                    $conversations[$i]["tags"] = $result[$j]["tags"];

                    // Include the phone number in the conversation data
                    if ($result[$j]["source"] == 'bm' || $result[$j]["source"] == 'tg' || $result[$j]["source"] == 'tw') {
                        $conversations[$i]["phone"] = ucfirst($result[$j][""]);
                    } else {
                        $conversations[$i]["phone"] = ($result[$j]["phone"] !== null) ? $result[$j]["phone"] : 'Steambox';
                    }
...
//             sb_users.id IN (SELECT user_id FROM sb_users_data WHERE value LIKE "%' . $search . '%" OR value LIKE "' . $search . '%") OR
...
function sb_search_conversations($search)
{
    $searchTerms = explode(' ', trim(sb_db_escape(mb_strtolower($search))));
    $conditions = [];

    foreach ($searchTerms as $term) {
        $term = sb_db_escape($term);
        $conditions[] = "(LOWER(sb_users.first_name) LIKE '%$term%' OR LOWER(sb_users.last_name) LIKE '%$term%')";
    }
    ...
    $result = sb_db_get(
        ...
        LEFT JOIN sb_users_data ON sb_users.id = sb_users_data.user_id" .
            ...
            LOWER(sb_users.email) LIKE '%$search%' OR
            LOWER(sb_conversations.title) LIKE '%$search%' OR 
            sb_users.id IN (SELECT user_id FROM sb_users_data WHERE value LIKE '%$search%') OR
            LOWER(sb_conversations.tags) LIKE '%$search%' OR
            LOWER(sb_conversations.label) LIKE '%$search%' OR
            $conditions
        )" .
            (is_numeric($search)
                ? " OR sb_conversations.id = $search OR
                   sb_conversations.department = $search OR
                   sb_conversations.agent_id = $search"
                : ""
            ) .
            " GROUP BY sb_users.id,sb_messages.conversation_id ORDER BY sb_messages.creation_time DESC",
        false
...
function sb_get_label_conversations($exclude_id = -1, $agent = false)
{
    $exclude = ($exclude_id != -1) ? " AND sb_messages.conversation_id <> " . sb_db_escape($exclude_id) : "";

    $labels = array("nuevos", "procesados", "pendientes", "resueltos", "perdidos");
    $labels_condition = implode("', '", $labels);
                ...
                <div id="CstBtn" class="cstdown-content">
                    ...
                    ?>
                        <a id="<?= $label ?>" class="sb-input-setting cst-a">
                            <i class="<?= "cst-i sb-icon sb-tags tags-" . $label ?>"></i>&nbsp;<?= $label ?>
