...
function sb_get_active_user(
    ...
    if ($login_app !== false) {
        ...
        //     $app == "default" &&
        //     (!$return ||
        //         sb_isset($return, "email") !=
        //         sb_isset($login_app_data, "email"))
        // ) {
        //     $return = sb_add_user_and_login(
        //         $login_app_data,
        //         sb_isset($login_app_data, "extra", []),
        //         false
        //     );
        //     if (
        //         sb_is_validation_error($return) &&
        //         $return->error == "duplicate-email" &&
        //         !empty($login_app_data["password"])
        //     ) {
        //         $active_user = sb_db_get(
        //             'SELECT id, token FROM sb_users WHERE password = "' .
        //                 sb_db_escape($login_app_data["password"]) .
        //                 '" AND email = "' .
        //                 sb_isset($login_app_data, "email", "") .
        //                 '" LIMIT 1'
        //         );
        ...
        //         ? array_merge($return[0], ["cookie" => $return[1]])
        //         : false;
        // } elseif (defined("SB_PERFEX") && $app == "perfex") {
        //     $return = sb_perfex_get_active_user_function(
        //         $return,
        //         $login_app_data
        //     );
        // } elseif (defined("SB_WHMCS") && $app == "whmcs") {
        //     $return = sb_whmcs_get_active_user_function(
        //         $return,
        //         $login_app_data
        //     );
        // } elseif (defined("SB_AECOMMERCE") && $app == "aecommerce") {
        //     $return = sb_aecommerce_get_active_user_function(
        //         $return,
        //         $login_app_data
        //     );
        // }
...
function sb_js_admin()
{
    ...
    $settings = [
        ...
        "notifications-icon" => $cloud
            ? SB_CLOUD_BRAND_ICON_PNG
            : sb_get_setting("notifications-icon", STMBX_URL . "/media/icon.png"),
        ...
        "login-time" => time(),
        "smart-reply" => sb_get_multi_setting(
            "dialogflow-smart-reply",
            "dialogflow-smart-reply-active"
        ),
        "single-agent" =>
        intval(
            sb_db_get(
    ...
    //     $settings["languages"] = json_encode(
    //         sb_isset(sb_wp_language_settings(), "languages", [])
    //     );
    // }
    // if (defined("SB_PERFEX")) {
    //     $settings["perfex-url"] = sb_get_setting("perfex-url");
    // }
    // if (defined("SB_WHMCS")) {
    //     $settings["whmcs-url"] = sb_get_setting("whmcs-url");
    // }
    // if (defined("SB_AECOMMERCE")) {
    //     $settings["aecommerce-panel-title"] = sb_get_setting(
    //         "aecommerce-panel-title",
    //         "Active eCommerce"
    //     );
    // }
    if ($settings["pusher"]) {
    ...
    if ($settings["smart-reply"]) {
        $settings["smart-reply-language-detection"] = sb_get_multi_setting(
            "dialogflow-smart-reply",
            "dialogflow-smart-reply-language-detection"
        );
        $settings["smart-reply-language-detection-bot"] =
            sb_get_setting("dialogflow-active") &&
            sb_get_multi_setting(
                "dialogflow-language-detection",
                "dialogflow-language-detection-active"
            );
        $settings["smart-reply-agent-assistant"] = !empty(sb_get_multi_setting(
            "dialogflow-smart-reply",
            "dialogflow-smart-reply-profile"
        ));
    }
    ...
    if ($cloud) {
        $cookie_cloud = json_decode(
            sb_encryption($_POST["cloud"], false),
            true
        );
        $settings["cloud"] = [
            "email" => $cookie_cloud["email"],
            "id" => sb_db_get(
                'SELECT id FROM sb_users WHERE email = "' .
                    $cookie_cloud["email"] .
                    '"'
            )["id"],
            "cloud_user_id" => $cookie_cloud["user_id"],
            "token" => $cookie_cloud["token"],
        ];
    }
    $translations =
    ...
    $code .= 'var SB_VERSIONS = {
        dialogflow: "' . (defined("SB_DIALOGFLOW") ? SB_DIALOGFLOW : -1) . '",
...
function sb_add_user(
    ...
    if (!empty($settings["email"])) {
        ...
        $existing_email = sb_db_get(
            'SELECT user_type FROM sb_users WHERE email = "' .
                $settings["email"] .
                '" LIMIT 1'
        );
        if ($existing_email) {
    ...
    if (
        ...
        sb_get_user_by("phone", $settings_extra["phone"][0]) &&
        !sb_get_setting("duplicate-emails")
    ) {
        return new SBValidationError("duplicate-phone");
    }
    if (empty($settings["profile_image"])) {
        $settings["profile_image"] = sb_get_avatar(
            $settings["first_name"],
            $settings["last_name"]
        );
    }
    // Add plus "+" symbol to phone number
    if (!empty($settings_extra["phone"])) {
        // Check if plus symbol already exists in the phone number
        if (strpos($settings_extra["phone"][0], '+') === false) {
            $settings_extra["phone"][0] = '+' . str_replace(' ', '', $settings_extra["phone"][0]);
        }
    }
    if (empty($settings["first_name"])) {
        $name = sb_get_setting("visitor-prefix");
        $settings["first_name"] =
            $name === false || $name == "" ? "User" : $name;
        $settings["last_name"] = "#" . rand(0, 99999);
    }
    if (empty($settings["user_type"])) {
        $settings["user_type"] = empty($settings["email"]) ? "visitor" : "user";
    } elseif (
        !in_array($settings["user_type"], [
            "visitor",
            "user",
            "lead",
            "agent",
    ...
    if ($settings["user_type"] == "user") {
        if (
            !empty($settings["first_name"]) &&
            substr($settings["last_name"], 0, 1) == "#"
        ) {
            $settings["last_name"] = "";
        }
    }
    ...
    $query =
        'INSERT INTO sb_users(first_name, last_name, password, email, profile_image, user_type, creation_time, token, department, last_activity) VALUES ("' .
    ...
    if (
        ...
        ($settings["user_type"] == "user" ||
            $settings["user_type"] == "lead" ||
            sb_get_setting("visitor-autodata"))
    ) {
    ...
    if (
        isset($_POST["payload"]) &&
        isset($_POST["payload"]["rich-messages"]) &&
        isset($_POST["payload"]["rich-messages"]["registration"])
    ) {
...
function sb_update_user(
    ...
    if ($email && sb_is_cloud() && sb_is_agent($user_type)) {
        $old_email = sb_isset(
            sb_db_get("SELECT email FROM sb_users WHERE id = " . $user_id),
            "email"
        );
...
function sb_get_users(
    ...
    $main_field_sorting = in_array($sorting_field, [
        "id",
        "first_name",
        "last_name",
        "email",
    ...
    $users = sb_db_get(
        SELECT_FROM_USERS .
            " FROM sb_users " .
            $query .
            sb_routing_and_department_db("sb_conversations", true) .
            ($main_field_sorting
                ? " ORDER BY " .
                sb_db_escape($sorting_field) .
                " " .
                sb_db_escape($sorting[1])
                : "") .
            " LIMIT " .
            intval(sb_db_escape($pagination, true)) * 25 .
            ",25",
        false
    );
        ...
        if ($extra && (!$is_array || count($extra))) {
            ...
            for ($i = 0; $i < $users_count; $i++) {
                ...
                $conversation = sb_db_get(
                    "SELECT * FROM sb_conversations A,sb_messages B WHERE A.id=B.conversation_id AND A.user_id=" .
                        $users[$i]["id"],
                    false
                );
            ...
            $users_extra = sb_db_get(
                "SELECT user_id, slug, value FROM sb_users_data WHERE user_id IN (" .
                    substr($query, 0, -1) .
                    ")" .
                    $query_extra .
                    " ORDER BY user_id",
        ...
        if (!$main_field_sorting) {
            if ($sorting[1] == "ASC") {
                usort($users, function ($a, $b) use ($sorting_field) {
                    return isset($a["extra"][$sorting_field])
                        ? $a["extra"][$sorting_field] <=>
                        (isset($b["extra"][$sorting_field])
                            ? $b["extra"][$sorting_field]
                            : "")
                        : -1;
                });
            } else {
                usort($users, function ($a, $b) use ($sorting_field) {
                    return isset($b["extra"][$sorting_field])
                        ? $b["extra"][$sorting_field] <=>
                        (isset($a["extra"][$sorting_field])
                            ? $a["extra"][$sorting_field]
                            : "")
                        : -1;
                });
...
//     $users = sb_db_get('SELECT A.* FROM sb_users A, sb_conversations B WHERE A.id=B.user_id AND user_type <> "bot" AND ' . (is_numeric($datetime) ? 'A.id > ' . $datetime : 'A.creation_time > "' . $datetime . '"') .
//     sb_routing_and_department_db("sb_conversations", true) . " ORDER BY A.id DESC", false);
//     if (isset($users) && is_array($users)) {
//     return $users;
//     } else {
...
function sb_search_users($search)
{
    ...
    $result = sb_db_get(
        'SELECT A.*, B.label FROM sb_users A,sb_conversations B,sb_messages C WHERE A.id=B.user_id AND B.id=C.conversation_id AND A.user_type <> "bot" AND (' .
            $query .
            ' A.email LIKE "%' .
            $search[0] .
            '%" OR A.id IN (SELECT user_id FROM sb_users_data WHERE value LIKE "%' .
            $search[0] .
            '%")) ' .
            sb_routing_and_department_db("sb_conversations", true) .
            " GROUP BY A.id;",
...
//         (ctype_digit($first_name[0]) || ctype_alpha($first_name[0])) &&
//         (ctype_digit($first_char_last_name) ||
//             ctype_alpha($first_char_last_name))
//     ) {
//         $file_name = rand(99, 9999999) . ".png";
//         $picture_url = sb_download_file(
//             "https://ui-avatars.com/api/?background=ffffff&size=512&font-size=0.35&name=" .
//                 $first_name .
//                 "+" .
//                 $last_name,
//             $file_name
...
{
    $file_name = rand(99, 9999999) . ".png";
    $image_url = "https://source.unsplash.com/random/250x250/?city";
        ...
        for ($i = 0; $i < count($conversations); $i++) {
            ...
            for ($j = 0; $j < count($result); $j++) {
                if ($conversation_id == $result[$j]["conversation_id"]) {
                    $conversations[$i]["first_name"] =
                        $result[$j]["first_name"];
                    $conversations[$i]["last_name"] = $result[$j]["last_name"];
                    $conversations[$i]["profile_image"] =
                        $result[$j]["profile_image"];
                    $conversations[$i]["user_id"] = $result[$j]["id"];
                    $conversations[$i]["conversation_status_code"] =
                        $result[$j]["status_code"];
                    $conversations[$i]["conversation_source"] =
                        $result[$j]["source"];
                    $conversations[$i]["user_type"] = $result[$j]["user_type"];
                    $conversations[$i]["label"] = $result[$j]["label"];
                    break;
                }
            }
        }
...
function sb_get_conversations(
    ...
    //         (sb_get_agent_department() === false && $department
    //             ? " AND C.department = " . sb_db_escape($department, true)
    //             : "") .
    ...
    //         $exclude_visitors .
    //         " GROUP BY conversation_id ORDER BY C.creation_time DESC,A.creation_time DESC LIMIT " .
    ...
    $result = sb_db_get(
        "SELECT A.*, B.user_type as message_user_type, C.title FROM sb_messages A, sb_users B, sb_conversations C WHERE B.id = A.user_id " .
            ...
            intval(sb_db_escape($pagination, true)) * 500 .
            ", 500", // load list conversations from database
        false
    );
...
function sb_get_new_conversations($datetime)
{
    ...
    $result = sb_db_get(
        "SELECT A.*, B.user_type as message_user_type, C.title FROM sb_messages A, sb_users B, sb_conversations C WHERE A.id IN (SELECT max(id) FROM sb_messages WHERE " .
            ...
            sb_routing_and_department_db("C") .
            " GROUP BY conversation_id ORDER BY A.creation_time DESC",
        false
    );
    if (isset($result) && is_array($result)) {
...
function sb_search_conversations($search)
{
    ...
    $result = sb_db_get(
        "SELECT sb_messages.*, sb_users.user_type as message_user_type, sb_conversations.title 
        FROM sb_messages
        JOIN sb_users ON sb_users.id = sb_messages.user_id
        JOIN sb_conversations ON sb_conversations.id = sb_messages.conversation_id
        LEFT JOIN sb_users_data ON sb_users.id = sb_users_data.user_id" .
            ...
            sb_users.id IN (SELECT user_id FROM sb_users_data WHERE value LIKE "%' . $search . '%" OR value LIKE "' . $search . '%")
        )' .
            (is_numeric($search)
                ? " OR sb_conversations.id = " . $search .
                " OR sb_conversations.department = " . $search .
                " OR sb_conversations.agent_id = " . $search
                : ""
            ) .
            " GROUP BY sb_users.id,sb_messages.conversation_id ORDER BY sb_messages.creation_time DESC",
        false
    );
...
function sb_get_tag_conversations($exclude_id = -1, $agent = false)
{
    $exclude =
        $exclude_id != -1
        ? " AND sb_messages.conversation_id <> " . sb_db_escape($exclude_id)
        : "";
    $query_part = false
        ? 'SELECT conversation_id FROM sb_messages WHERE user_id IN (SELECT user_id FROM sb_conversations A, sb_users B WHERE B.id = A.user_id AND (label="cd" OR label="wm" OR label="cw" OR label="cl" OR label="ht"))'
        : 'SELECT id FROM sb_conversations WHERE user_id IN (SELECT user_id FROM sb_conversations A, sb_users B WHERE B.id = A.user_id AND (label="cd" OR label="wm" OR label="cw" OR label="cl" OR label="ht"))';
    return sb_db_get(
        "SELECT sb_messages.*, sb_users.first_name, sb_users.last_name, sb_users.profile_image, sb_users.user_type, sb_conversations.user_id,sb_conversations.status_code AS conversation_status_code, sb_conversations.department, sb_conversations.agent_id, sb_conversations.title FROM sb_messages, sb_users, sb_conversations WHERE sb_users.id = sb_messages.user_id" .
            sb_routing_and_department_db() .
            ' AND sb_messages.conversation_id = sb_conversations.id AND sb_messages.id IN (SELECT max(sb_messages.id) FROM sb_messages, sb_conversations WHERE (sb_messages.message <> "" OR sb_messages.attachments <> "") AND sb_messages.conversation_id = sb_conversations.id' .
            ($agent ? "" : "") .
            $exclude .
...
function sb_get_last_conversation_id_or_create(
    $user_id,
    $conversation_status_code = 1
) {
    $conversation_id = sb_isset(
        sb_db_get(
            "SELECT id FROM sb_conversations WHERE user_id = " .
                sb_db_escape($user_id, true) .
    ...
    if ($conversation_id) {
        ...
        return sb_isset(
            sb_isset(
                sb_new_conversation($user_id, $conversation_status_code),
                "details"
            ),
            "id"
        );
...
function sb_new_conversation(
    ...
    } elseif (defined("SB_AECOMMERCE")) {
        $agent_id = sb_aecommerce_get_agent_id($agent_id);
    }
    $user_id = sb_db_escape($user_id, true);
    $conversation_id = sb_db_query(
        "INSERT INTO sb_conversations(user_id, title, status_code, creation_time, department, agent_id, source, extra) VALUES (" .
            $user_id .
            ', "' .
...
function sb_update_conversation_label($conversation_id, $label)
{
    $conversation_id = sb_db_escape($conversation_id, true);
    $agent = sb_is_agent();
    $q =
...
function sb_direct_message($user_ids, $message, $subject = false, $template = false)
{
    ...
    if ($template == "true") {
        ...
        if (count($user_ids) != count($wa_to)) {
            ...
            return new SBValidationError("Please Select All Users Whatsapp");
        }
...
function sb_send_message(
    ...
    if ($sender_id != -1) {
        ...
        $is_human_takeover_active =
            $is_dialogflow_active &&
            sb_dialogflow_is_human_takeover($conversation_id);
        $is_human_takeover =
            $is_dialogflow_active &&
            !$is_human_takeover_active &&
            sb_get_multi_setting(
        ...
        if (
            ...
            sb_send_message(sb_get_bot_id(), $conversation_id, "", [], false, [
                "human-takeover" => true,
            ]);
        ...
        if (
            ...
            if (
                sb_is_cloud() &&
                in_array(sb_defined("SB_CLOUD_MEMBERSHIP_TYPE", "messages"), [
                    "messages",
                    "messages-agents",
                ])
            ) {
            ...
            return [
                "id" => $response,
                "queue" => $queue,
                "human_takeover_active" => $is_human_takeover_active,
...
function sb_init_translations()
{
    ...
    if (!empty($SB_LANGUAGE) && $SB_LANGUAGE[0] != "en") {
        ...
        if (sb_is_cloud()) {
            $cloud_path =
                SB_PATH .
                "/uploads/cloud/languages/" .
                sb_isset(sb_cloud_account(), "user_id") .
                "/" .
                $SB_LANGUAGE[1] .
                "/" .
                $SB_LANGUAGE[0] .
        ...
        $cloud_default_language = sb_defined(
            "SB_CLOUD_DEFAULT_LANGUAGE_CODE",
            false
        );
        ...
        if (
            ...
            ((!$admin && !$auto_translation) ||
                ($admin && !sb_get_setting("admin-auto-translations")))
        ) {
            $language = $cloud_default_language;
        }
        if (
            !(!$language ||
                $language == "en" ||
    ...
    foreach ($translations as $key => $translation) {
        foreach ($translation as $key_area => $translations_list) {
            $json = html_entity_decode(
                json_encode($translations_list, JSON_INVALID_UTF8_IGNORE)
            );
            if ($json) {
                if ($is_cloud) {
                    sb_file(
                        $cloud_path . "/" . $key_area . "/" . $key . ".json",
                        $json
                    );
                } else {
...
function sb_restore_user_translations()
{
    ...
    foreach ($translations_user as $key => $translations) {
        for ($i = 0; $i < count($paths); $i++) {
            $path = $paths[$i];
            if (
                isset($translations_all[$key]) &&
                isset($translations_all[$key][$path])
            ) {
                foreach ($translations_all[$key][$path]
                    as $key_two => $translation) {
                    if (!isset($translations[$path][$key_two])) {
                        $translations[$path][$key_two] =
                            $translations_all[$key][$path][$key_two];
                    }
                }
            }
            sb_file(
                SB_PATH .
                    ...
                    $key .
                    ".json",
                json_encode($translations[$path], JSON_INVALID_UTF8_IGNORE)
            );
        }
    }
}
...
function sb_get_admin_language($user_id = false)
{
    ...
    return $language && $language != "en"
        ? $language
        : sb_defined("SB_CLOUD_DEFAULT_LANGUAGE_CODE", $language);
}
...
function sb_get_setting_code($setting)
{
    if (isset($setting)) {
        ...
        $content =
            ...
            (isset($setting["help"]) &&
                (!sb_is_cloud() || defined("SB_CLOUD_DOCS"))
                ? '<a href="' .
                (defined("SB_CLOUD_DOCS")
                    ? SB_CLOUD_DOCS .
                    substr(
                        $setting["help"],
                        strpos($setting["help"], "#")
                    ...
                    : $setting["help"]) .
                '" target="_blank" class="sb-icon-help"></a>'
                : "") .
        ...
        switch ($type) {
            ...
            case "article":
                $content .= '
                <div class="sb-code-color">
                <code style="color:gray;">// Ejemplo: https://usuario.steambox.dev/</code>
                <br> 
                <code style="color:#963852;">&lt;script id="steambox" src="js/main.js"&gt;<br>
                &lt;/script&gt;</code>
                <br>
                </div>';
                break;
...
function sb_get_front_settings()
{
    ...
    $return = [
        ...
        "notifications-icon" => sb_is_cloud()
            ? SB_CLOUD_BRAND_ICON_PNG
            : sb_get_setting("notifications-icon", STMBX_URL . "/media/icon.png"),
        "bot-id" => sb_get_bot_id(),
        ...
        "martfury" =>
        defined("SB_MARTFURY") && sb_get_setting("martfury-private")
            ? sb_get_setting("martfury-linking")
            : [],
    ...
    if ($return["pusher"]) {
        $return["pusher-key"] = $return["cloud"]
            ? CLOUD_PUSHER_KEY
            : sb_get_multi_setting("pusher", "pusher-key");
        $return["pusher-cluster"] = $return["cloud"]
            ? CLOUD_PUSHER_CLUSTER
            : sb_get_multi_setting("pusher", "pusher-cluster");
    }
    ...
    if (defined("SB_WOOCOMMERCE")) {
        ...
        $return["woocommerce-returning-visitor"] =
            !in_array(sb_isset($active_user, "user_type"), [
                "user",
                "agent",
                "admin",
            ]) &&
            sb_get_multi_setting(
                "wc-returning-visitor",
                "wc-returning-visitor-active"
            );
    }
...
//         }
//         sb_db_query(
//             'DELETE FROM sb_settings WHERE name LIKE "articles-translations-%" AND name NOT IN (' .
//                 substr($db, 0, -1) .
//                 ")"
//         );
//     }
...
//         $language = false;
//     }
//     $articles = sb_get_external_setting(
//         $language && !$languages_all
//             ? "articles-translations-" . $language
//             : "articles"
//     );
//     $articles_translations = [];
//     $return = [];
//     $category =
//         $categories && $categories != "true" && !is_bool($categories)
//             ? $categories
//             : false;
...
//                         in_array(
//                             $category,
//                             sb_isset($article, "categories", [])
//                         ) ||
//                         $category == sb_isset($article, "parent_category")
//                     ) {
...
//     if ($languages_all) {
//         $rows = sb_db_get(
//             'SELECT name, value FROM sb_settings WHERE name LIKE "articles-translations-%"',
//             false
//         );
//         for ($i = 0; $i < count($rows); $i++) {
//             $articles_translations[substr($rows[$i]["name"], -2)] = json_decode(
//                 $rows[$i]["value"],
//                 true
//             );
//         }
//     }
//     return $categories === true || $categories == "true" || $languages_all
//         ? [$return, sb_get_articles_categories(), $articles_translations]
//         : $return;
// }
...
// {
//     $articles = sb_get_external_setting(
//         $language ? "articles-translations-" . $language : "articles"
//     );
//     $return = [];
...
//         for ($i = 0; $i < count($articles); $i++) {
//             if (
//                 strpos(strtolower($articles[$i]["title"]), $search) !== false ||
//                 strpos(strtolower($articles[$i]["content"]), $search)
//             ) {
//                 $articles[$i]["content"] = mb_substr(
//                     $articles[$i]["content"],
//                     0,
//                     100
//                 );
...
//     $ratings = sb_isset(
//         sb_db_get(
//             'SELECT value FROM sb_reports WHERE name = "article-ratings" AND extra = "' .
//                 sb_db_escape($article_id) .
//                 '" AND creation_time = "' .
//                 $now .
//                 '"'
//         ),
...
//         if (empty($ratings)) {
//             return sb_db_query(
//                 'INSERT INTO sb_reports (name, value, creation_time, external_id, extra) VALUES ("article-ratings", "[' .
//                     $rating .
//                     ']", "' .
//                     $now .
//                     '", NULL, "' .
...
//                     json_encode($ratings) .
//                     '" WHERE name = "article-ratings" AND extra = "' .
//                     $article_id .
//                     '"'
//             );
//         }
...
function sb_email_send($to, $subject, $body, $sender_suffix = "")
{
    ...
    if (!$host && sb_is_cloud()) {
        $settings = [
            "email-server-host" => CLOUD_SMTP_HOST,
            "email-server-user" => CLOUD_SMTP_USERNAME,
            "email-server-password" => CLOUD_SMTP_PASSWORD,
            "email-server-from" => CLOUD_SMTP_SENDER,
            "email-sender-name" => CLOUD_SMTP_SENDER_NAME,
            "email-server-port" => CLOUD_SMTP_PORT,
        ];
    }
    if (empty($to)) {
        return false;
                                ...
                                if (!empty($message)) {
                                    ...
                                    if (!$agent) {
                                        if (
                                            ...
                                            defined("SB_SLACK") &&
                                            sb_slack_can_send($conversation_id)
                                        ) {
...
{
    return (defined("SB_UPLOAD_PATH") &&
        SB_UPLOAD_PATH &&
        defined("SB_UPLOAD_URL") &&
        SB_UPLOAD_URL
        ? ($url
            ? SB_UPLOAD_URL
            : SB_UPLOAD_PATH)
        : ($url ? STMBX_URL . "/" : SB_PATH . "/") . "uploads") .
        ($date ? "/" . date("d-m-y") : "");
}
 ...
 * 4. Update Steambox and all apps
...
function sb_css(
    ...
    if ($color_1) {
        $css .=
            ...
            "; }";
        $css .=
            '.sb-chat .sb-dashboard>div>.sb-btn,.sb-search-btn>input,.sb-input>input:focus, .sb-input>select:focus, .sb-input>textarea:focus,
                 .sb-input.sb-input-image .image:hover { border-color: ' .
            $color_1 .
            "; }";
        $css .=
            ...
            "; }";
        $css .=
            ".sb-search-btn>input:focus,.sb-input>input:focus, .sb-input>select:focus, .sb-input>textarea:focus,.sb-input.sb-input-image .image:hover { box-shadow: 0 0 5px rgba(104, 104, 104, 0.2); }";
        $css .=
...
function sb_system_requirements()
{
    ...
    // cURL
    $checks["curl"] =
        function_exists("curl_version") && is_array(sb_get_versions());
...
?>
    <div class="sb-editor" id="controls">
        ...
        <div class="sb-bar-icons sb-hide">
            ...
            <div class="tagDown">
                <div onclick="ExtraButton()" class="cst-btn sb-icon-tag"></div>
                <div id="CstBtn" class="cstdown-content">
                    <?php foreach (sb_get_setting("label-names") as $key => $label) { ?>
                        <a id="<?= $key ?>" class="sb-input-setting cst-a">
                            <i class="<?= "cst-i sb-icon sb-tags tags-" . $key ?>">
                            </i>&nbsp;<?php if (empty($label)) {
                                            echo "Crear etiqueta";
                                        } else {
                                            sb_e($label);
                                        } ?></a>
...
// function sb_execute_bot_message(
//     $name,
//     $conversation_id,
...
//             $valid =
//                 $settings["chat-timetable-active"] &&
//                 (!sb_office_hours() ||
//                     (!$settings["chat-timetable-agents"] &&
//                         !sb_agents_online()));
//             break;
//         case "follow_up":
...
//                     "chat-timetable",
//                     "chat-timetable-title"
//                 );
//                 $message =
//                     ($message ? "*" . sb_($message) . "*" . PHP_EOL : "") .
//                     sb_(
//                         sb_get_multi_setting(
//                             "chat-timetable",
//                             "chat-timetable-msg"
...
//             case "follow_up":
//                 $message =
//                     '[email id="sb-follow-up" title="' .
//                     sb_rich_value($settings["title"]) .
//                     '" message="' .
//                     sb_rich_value($settings["message"]) .
//                     '" placeholder="' .
//                     sb_rich_value($settings["placeholder"]) .
//                     '" name="' .
//                     $settings["name"] .
//                     '" last-name="' .
//                     $settings["last-name"] .
...
//                     '" success="' .
//                     sb_rich_value($settings["success"]) .
//                     '"]';
//                 break;
//             case "subscribe":
//                 $message =
...
//                     sb_rich_value($settings["subscribe-title"]) .
//                     '" message="' .
//                     sb_rich_value($settings["subscribe-msg"]) .
//                     '" success="' .
//                     sb_rich_value($settings["subscribe-msg-success"]) .
//                     '" placeholder="' .
//                     sb_rich_value(
//                         sb_isset($settings, "subscribe-placeholder", "Email"),
//                         false
//                     ) .
//                     '" name="' .
//                     ($settings["subscribe-name"] ? "true" : "false") .
//                     '" last-name="' .
//                     ($settings["subscribe-last-name"] ? "true" : "false") .
//                     '"]';
//                 break;
//         }
//         $message_id = sb_send_message(
...
//                 $name . "_message" => true,
//                 "auto_reply" => 1,
//                 "preview" => $last_user_message ? $last_user_message : $message,
//             ]
//         )["id"];
//         return [
//             "message" => $message,
...
function sb_messaging_platforms_functions(
    ...
    $dialogflow_active =
        defined("SB_DIALOGFLOW") && sb_get_setting("dialogflow-active");
    $skip_dialogflow = false;
    $slack =
        defined("SB_SLACK") && sb_slack_can_send($conversation_id)
        ? [$user["id"], sb_get_user_name($user), $user["profile_image"]]
        : false;
    $message_id = false;
    ...
    if ($dialogflow_active) {
        ...
        if (
            !$skip_dialogflow &&
            (!sb_get_setting("dialogflow-timetable") || !sb_office_hours())
        ) {
            $response = sb_dialogflow_message(
                $conversation_id,
                $message,
                -1,
                [sb_get_user_language($user_id)],
                $attachments
            );
            $messages = sb_isset($response, "messages", []);
            if (isset($response["human_takeover"])) {
                $human_takeover = true;
            }
            for ($i = 0; $i < count($messages); $i++) {
                ...
                if ($message_text || isset($messages[$i]["attachments"])) {
                    if (
                        ($source_name == "tm" || $source_name == "em") &&
                        !sb_dialogflow_is_unknow($response)
                    ) {
                        if ($source_name == "em") {
                            sb_email_create(
                                $user["id"],
                                sb_get_setting("bot-name", "Dialogflow"),
                                sb_get_setting("bot-image"),
                                $message_text,
                                $message_attachments,
                                false,
                                $conversation_id
                ...
                if ($slack) {
                    sb_send_slack_message(
                        ...
                        $slack[2],
                        $messages[$i]["message"],
                        sb_isset($messages[$i], "attachments", []),
                        $conversation_id
                    );
                }
            }
...
function sb_messaging_platforms_send_message(
    ...
    switch ($conversation["source"]) {
        ...
        case "wc":
            return sb_wechat_send_message(
                $platform_value
                    ? $platform_value
                    : sb_get_user_extra($user_id, "wechat-id"),
                $message,
                $attachments
            );
        case "tw":
...
function sb_send_sms(
    ...
    if ($attachments) {
        ...
        for ($i = 0; $i < count($attachments); $i++) {
            $attachment = is_array($attachments[$i])
                ? $attachments[$i][1]
                : $attachments[$i];
            if (
                in_array(pathinfo($attachment, PATHINFO_EXTENSION), $mime_types)
            ) {
...
function sb_reports($report_name, $date_start = false, $date_end = false)
{
    ...
    switch ($report_name) {
        case "conversations":
            ...
            break;
        case "tagged-conversations":
            $tags =
                'label="cd" OR label="wm" OR label="cw" OR label="cl" OR label="ht"';
            $query =
                'SELECT * FROM sb_conversations A, sb_users B WHERE B.id = A.user_id AND (label="cd" OR label="wm" OR label="cw" OR label="cl" OR label="ht")';
            $extra = sb_get_tag_conversations();
        ...
        case "follow-up":
            $query =
                'SELECT creation_time, value FROM sb_reports A WHERE name = "follow-up"';
        ...
        case "articles-searches":
            $query =
                'SELECT creation_time, value FROM sb_reports A WHERE name = "articles-searches"';
        ...
        case "articles-ratings":
            $query =
                'SELECT value, extra FROM sb_reports A WHERE name = "article-ratings"';
        ...
        case "articles-views":
            $query =
                'SELECT creation_time, value, extra FROM sb_reports A WHERE name = "articles-views"';
            $title = "Article views";
            $description =
    ...
    switch ($report_name) {
        ...
        case "articles-views-single":
            ...
            for ($i = 0; $i < count($rows); $i++) {
                $date_row = $single
                    ? $rows[$i]["extra"]
                    : date("d/m/Y", strtotime($rows[$i]["creation_time"]));
                $data[$date_row] = [
                    intval($rows[$i]["value"]) +
                        (empty($data[$date_row]) ? 0 : $data[$date_row][0]),
                ];
            }
            if ($single) {
                $articles = sb_get_articles();
                ...
                for ($i = 0; $i < count($articles); $i++) {
                    ...
                    if (isset($data[$id])) {
                        ...
                        $data_names[strlen($article_title) > 40
                            ? substr($article_title, 0, 40) . "..."
                            : $article_title] = $data[$id];
                    }
                }
                $data = $data_names;
            }
...
function sb_reports_update(
    ...
    switch ($name) {
        ...
        case "articles-views":
            $where =
                ' WHERE name = "articles-views" AND extra = "' .
                $extra .
                '" AND creation_time = "' .
                $now .
                '"';
