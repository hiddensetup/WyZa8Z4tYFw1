...
define("SB_VERSION", "STEAMBOXCHAT");
...
function sb_update_user(
    ...
    // Update login details and perform autodata update if necessary
    $active_user = sb_get_active_user();
...
function sb_get_users($sorting = ['creation_time', 'DESC'], $user_types = [], $search = '', $pagination = 0, $extra = false, $user_ids = false)
{
    ...
    $users = sb_db_get(SELECT_FROM_USERS . ' FROM sb_users ' . $query . ($main_field_sorting ? (' ORDER BY ' . sb_db_escape($sorting_field) . ' ' . sb_db_escape($sorting[1])) : '') . ' LIMIT ' . (intval(sb_db_escape($pagination, true)) * 50) . ',50', false);
